...
XO<- list.files(paste0(RCP,"/",model,"/"), pattern =paste0("*.s.tif$",sep=""))
ManyRunsStack<-raster::stack(paste0(RCP,"/",model,"/",XO))
...
Ecoregion <- raster(paste0(RCP,"/","Ecoregion100f.tif"))

#Read in roads file
roads <- raster(paste0(RCP,"/","road.tif",sep=""))
...
Year0<-list.files(paste0(RCP,"/",model,"/",replicate),pattern=(".img$"))

#paste0("inputs/", model, "/",model,replicate,"/")

Longleaf_Stack<-raster(paste0(RCP,"/",model,"/",replicate,"/",Year0[Year0 %in% paste0("bio-", Longleaf,"-",i,".img")]))
Loblolly_Stack<-raster(paste0(RCP,"/",model,"/",replicate,"/",Year0[Year0 %in% paste0("bio-", Loblolly,"-",i,".img")]))
Pine_Stack<-raster::stack(paste0(RCP,"/",model,"/",replicate,"/",Year0[Year0 %in% paste0("bio-", Pine,"-",i,".img")]))
Hardwood_Stack<-raster::stack(paste0(RCP,"/",model,"/",replicate,"/",Year0[Year0 %in% paste0("bio-", Hardwood,"-",i,".img")]))
Total<-raster(paste0(RCP,"/",model,"/",replicate,"/",Year0[Year0 %in% paste0("bio-TotalBiomass-", i, ".img")]))
...
median0 <- raster(paste0(RCP,"/",model,"/",replicate,"/",Year0[Year0 %in% paste0("AGE-MED-", i, ".img")]))
...
LU0 <- raster(paste0(RCP,"/",model,"/land-use-",i,".tif"))
...
#land use types
test_raster2[LU0 == 7] <- (1/90) #cropland
test_raster2[LU0 == 6] <- (1/90) #hay/pasture
test_raster2[LU0 == 5] <- (1/90) #grazing
test_raster2[LU0 == 0] <- (1/100) #water
test_raster2[LU0 == 1] <- (1/90) #reservoirs
test_raster2[LU0 == 2] <- (1/70) #wetlands
test_raster2[LU0 == 14] <- (1/100) #urban, high intensity
test_raster2[LU0 == 13] <- (1/100) #urban, low intensity
test_raster2[LU0 == 12] <- (1/90) #suburban
test_raster2[LU0 == 11] <- (1/80) #exurban, high intensity
test_raster2[LU0 == 10] <- (1/70) #exurban, low intensity
test_raster2[LU0 == 8] <- (1/100) #mining
test_raster2[LU0 == 15] <- (1/100) #commercial
test_raster2[LU0 == 16] <- (1/100) #industrial
test_raster2[LU0 == 18] <- (1/100) #transportation
...
write.table(maketext, file=paste0(RCP,"/",model,"/",replicate,"/","Output/nodes_",RCP,model,replicate,"yr",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
...
write.table(costcomplete, file=paste0(RCP,"/",model,"/",replicate,"/","Output/distance_",RCP,model,replicate,"yr",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
...
write.table(results, file=paste0(RCP,"/",model,"/",replicate,"/","Output/Metrics_",RCP,model,replicate,"yr",i,".txt"), sep = "\t", row.names = TRUE, col.names = TRUE)
