...
biore80geo <- raster(paste("R:/fer/rschell/Mozelewski/NECN_Tina/NECN_Tina_basic_geo_strat2/output/bio-reclass1-80.img",sep=""))
...
median80 <- raster(paste("R:/fer/rschell/Mozelewski/NECN_Tina/NECN_Tina_basic_geo_strat2/output/cohort-stats/AGE-MED-80.img",sep=""))
...
LU80 <- raster(paste("R:/fer/rschell/Mozelewski/NECN_Tina/NECN_Tina_basic_geo_strat2/land-use-80.tif", sep=""))
plot(LU80)
table(getValues(LU80))

#Create a raster that will become resistance raster
test_raster <- raster(paste("R:/fer/rschell/Mozelewski/NECN_Tina/NECN_Tina_basic_geo_strat2/output/bio-reclass1-80.img",sep=""))
...
writeOGR(obj=pol1, dsn="shapefiles", layer="pol80geo", driver="ESRI Shapefile")
...
pol1geo2_dis <- readOGR(dsn="shapefiles", "pol80geo_dis")
#pol1_dis$Area <- area(pol1_dis) #will prob need to remove this for subsequent model outputs
pol1geo2_dis$area_ha <- area(pol1geo2_dis)/10000
pol1geo2_dis$NEWid <- seq(from= 10001, to= 13726, by =1) #have to change this number every time. How to fix??

pol1geo2_dis$num1 <- seq(from = 1, to= length(pol1geo2_dis), by=1)
pol1geo2_dis$num2 <- seq(from = 1, to= length(pol1geo2_dis), by=1)

#Assign weight to habitat by type and area to be used in Conefor
pol1geo2_dis$weight <- NA
#polAll$weight[polAll$NEWid < 20000 & polAll$area_ha %in% c(21:35)] <- 56
pol1geo2_dis$weight[pol1geo2_dis$area_ha <=21 ] <- 60
pol1geo2_dis$weight[pol1geo2_dis$area_ha > 21] <- 85
pol1geo2_dis$weight[pol1geo2_dis$area_ha >= 35] <- 100
...
write.table(maketext, file = "nodes80geo2.txt", sep = "\t",
            row.names = FALSE, col.names = FALSE)

pol1rast<- rasterize(pol1geo2_dis, test_raster, field= "NEWid")
plot(pol1rast)


writeRaster(pol1rast,"R:/fer/rschell/Mozelewski/Chapter_2_analysis/pol1_cir_raster.tif", 
            format = "ascii", overwrite=TRUE) ###for circuitscape resistance layer
...
trueCentroidsgeo <- gCentroid(pol1geo2_dis, byid=TRUE, id = pol1geo2_dis$NEWid)
...
#resDist <- commuteDistance(test_trR, trueCentroidsgeo)
#spDist <- rSPDistance(test_trR, trueCentroidsgeo, trueCentroidsgeo, 1)
...
lookup <- cbind(pol1geo2_dis$NEWid, pol1geo2_dis$num1, pol1geo2_dis$num2)
...
write.table(costdist5000, file="costdistance80geo.txt", row.names=FALSE, col.names=FALSE)

#plot habitat patches + plot lines bewteen EUD-approved patches + plot background raster