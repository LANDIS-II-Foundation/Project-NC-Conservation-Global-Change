...
### Create conductance surface (1/resistance) for generalist species
...
test_raster[biore80 == 2 & median80 %in% c(11:20)] <- (1/30)
test_raster[biore80 == 2 & median80 %in% c(21:34)] <- (1/20)
test_raster[biore80 == 2 & median80 >= 35] <- (1/10)

#pine plantation community type
test_raster[biore80 == 3 & median80 %in% c(0:5)] <- (1/90)
test_raster[biore80 == 3 & median80 %in% c(6:10)] <- (1/80)
test_raster[biore80 == 3 & median80 %in% c(11:20)] <- (1/70)
test_raster[biore80 == 3 & median80 %in% c(21:30)] <- (1/60)
test_raster[biore80 == 3 & median80 >= 31] <- (1/50)
...
test_raster[biore80 == 4 & median80 %in% c(21:30)] <- (1/30)
test_raster[biore80 == 4 & median80 >= 31] <- (1/20)
...
pol1test <- rasterToPolygons(test_raster2, fun=function(x){x>= 0.03})
plot(pol1)
crs(pol1)

setwd("R:/fer/rschell/Mozelewski/Chapter_2_analysis/")
writeOGR(obj=pol1test, dsn="shapefiles", layer="pol80gentest", driver="ESRI Shapefile")
