...
#i = 40 #time step- change this! variables are 0:80 by 10
...
LU0 <- raster(paste0(RCP,"/", model,"/land-use-", i, ".tif",sep=""))

#use to not incorporate land use change, but establish BAU land use types
#LU0 <- raster(paste0(RCP,"/LUC/","land-use-",i,".tif"))
...
if (i > 0) {
  ...
  this_time <- raster(paste0(RCP,"/",model,"/CC",replicate,"_",i,"s.tif"))
  last_time <- raster(paste0(RCP,"/",model,"/CC",replicate,"_",i2,"s.tif"))
