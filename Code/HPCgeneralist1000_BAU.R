# R script:  HPCspecialist1000.R
# Connectivity script for habitat specialists with 1000m max dispersal distance
# Author:  Tina Mozelewski
# Usage:
#  Rscript HPCspecialist1000.R [#timesteps] [#models] [#replicates]
...
models<- c("cluster","economic","geodiversity","random")
#models<- "cluster"
...
u = 1000 #euclidean distance threshold- change this! variables are 1000 & 1000
#model<- "LUC"
#RCP = "CC45"
#replicate= "bcc"
...
Year0<-list.files(paste0(model,"/",replicate,"/biomass"),pattern=(".img$"))

#paste0("inputs/", model, "/",model,replicate,"/")

Longleaf_Stack<-raster(paste0(model,"/",replicate,"/biomass/",Year0[Year0 %in% paste0("bio-", Longleaf,"-",i,".img")]))
Loblolly_Stack<-raster(paste0(model,"/",replicate,"/biomass/",Year0[Year0 %in% paste0("bio-", Loblolly,"-",i,".img")]))
Pine_Stack<-raster::stack(paste0(model,"/",replicate,"/biomass/",Year0[Year0 %in% paste0("bio-", Pine,"-",i,".img")]))
Hardwood_Stack<-raster::stack(paste0(model,"/",replicate,"/biomass/",Year0[Year0 %in% paste0("bio-", Hardwood,"-",i,".img")]))
Total<-raster(paste0(model,"/",replicate,"/biomass/",Year0[Year0 %in% paste0("bio-TotalBiomass-", i, ".img")]))
...
median0 <- raster(paste0(model,"/",replicate,"/cohort-stats/AGE-MED-", i, ".img"))
...
test_raster[test_stack == 3 & median0 %in% c(6:10),] <- (1/50)
test_raster[test_stack == 3 & median0 %in% c(11:20),] <- (1/20)
test_raster[test_stack == 3 & median0 %in% c(21:34),] <- 1
...
test_raster[test_stack == 2 & median0 %in% c(11:20),] <- (1/50)
test_raster[test_stack == 2 & median0 %in% c(21:30),] <- (1/40)
test_raster[test_stack == 2 & median0 >= 31,] <- (1/30)
...
test_raster[test_stack == 5 & median0 %in% c(21:30),] <- (1/50)
test_raster[test_stack == 5 & median0 >= 31,] <- (1/40)
...
if (i > 0) {
  ...
  this_time <- raster(paste0(model,"/rep",replicate,"_",i,"g.tif"))
  last_time <- raster(paste0(model,"/rep",replicate,"_",i2,"g.tif"))
  ...
  write.table(maketext, file=paste0("Output_generalist1000/nodes_",model,replicate,"yr",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
  rm(pol8)
  rm(pol5)
  rm(a)
  rm(b)
...
EU1000<-EUnew[!(EUnew$value > 1000),]
EU1000_nodups <- EU1000[!duplicated(data.frame(list(do.call(pmin,EU1000),do.call(pmax,EU1000)))),]

rm(EUpts)

#merge 
colnames(EU1000_nodups) <- c("num1", "num2", "EUD")
lookup <- cbind(pol1_dis$ID, pol1_dis$num1, pol1_dis$num2)
colnames(lookup) <- c("ID", "num1", "num2")
EU_test <- merge(x = EU1000_nodups, y = lookup, by = "num1", all.x = TRUE)
...
costdist1000 <- merge(EU_fin_df, costmatrix, by.x= c("X2", "X1"), by.y = c("X1", "X2"))
costdist1000df <- data.frame(costdist1000)
costcomplete <- costdist1000df[!is.infinite(rowSums(costdist1000df)),]
write.table(costcomplete, file=paste0("Output_generalist1000/distance_",model,replicate,"yr",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
...
write.table(results, file=paste0("Output_generalist1000/Metrics_",model,replicate,"yr",i,".txt"), sep = "\t", row.names = TRUE, col.names = TRUE)
